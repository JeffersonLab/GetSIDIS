...
#include "SIDIS_Lite_LO.h" //this version doesn't include LHAPDF
using namespace std;
...
int main(){    
    ...
    for (Int_t j=0;j<Q2bin;j++){
       ...
       if(fA ==12){
            new_filename=Form("./c12_pion_LO_free_noPt/EIC_A12_pion_10_600_skim%d_wide_free_noPt_new.root",j);
            t1->Add(new_filename.Data());
            cerr<<Form("-- Reading in file: %s", new_filename.Data())<<endl;
            new_filename=Form("./c12_pion_LO_free_noPt/EIC_A12_pion_10_600_skim%d_wide_free_noPt.root",j);
            t1->Add(new_filename.Data());
            cerr<<Form("-- Reading in file: %s", new_filename.Data())<<endl;
       }
        if(fA ==2){
            //new_filename=Form("./d2_pion_LO_free_noPt/EIC_A2_pion_10_100_skim%d_wide_free_noPt_new.root",j);
            //t1->Add(new_filename.Data());
            cerr<<Form("-- Reading in file: %s", new_filename.Data())<<endl;
            new_filename=Form("./d2_pion_LO_free_noPt/EIC_A2_pion_10_100_skim%d_wide_free_noPt.root",j);
            t1->Add(new_filename.Data());
            cerr<<Form("-- Reading in file: %s", new_filename.Data())<<endl;
        }
        ...
        //TString xbj_cut = "(x>0.08 && x<=0.12)";
        TString xbj_cut = "(x>0.00 && x<=0.3)";

        ofstream outf;
        if(fA ==12)
            outf.open(Form("c12_output_Q2_%d_lo_wide_free_noPT.dat",j));
        if(fA ==2)
            outf.open(Form("d2_output_Q2_%d_lo_wide_free_noPT.dat",j));


        outf<<Form("%4s %10s %10s %10s %10s %10s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s",
                "#bin","Q2","x", "W", "z", "pt", "xs_inc","xs_pip","xs_pim","N_inc","N_pip","N_pim",
                "mulp_pip","mulp_pim","Asym","Astat", "Ax","u_avg","ub_avg","d_avg","db_avg","s_avg","sb_avg","g_avg", "u","ubar","d","dbar","s","sbar","g", "u_c", "ubar_c", "d_c","dbar_c","s_c","sbar_c","g_c","N_mc")<<endl;/*}}}*/
    ...
    /*}}}*/
        ...
        for (Int_t i=0;i<zbin;i++){
            ...
            TString CUT_unom=Form("(%s &&%s && z>%f && z<=%f)", xbj_cut.Data(), Q2_cut.Data(),z_min,z_max); 
            ...
            outf<<Form("%4d %10.4f %10.4f %10.4f %10.4f %10.4f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e",
                    i,                   //0
                    h1Q2->GetMean(),     //1
                    h1x->GetMean(),      //2
                    h1W->GetMean(),      //3
