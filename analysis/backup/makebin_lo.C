...
int main(){    
 ...
 //       TFile *f1 = new TFile(new_filename,"r");
  //      TTree *t1 = (TTree*) f1->Get("T");
       ...
       if(fA ==12){
            new_filename=Form("./c12_pion_LO_noPt/EIC_A12_pion_10_600_skim%d_wide_noPt_new.root",j);
            t1->Add(new_filename.Data());
            cerr<<Form("-- Reading in file: %s", new_filename.Data())<<endl;
            new_filename=Form("./c12_pion_LO_noPt/EIC_A12_pion_10_600_skim%d_wide_noPt.root",j);
            t1->Add(new_filename.Data());
            cerr<<Form("-- Reading in file: %s", new_filename.Data())<<endl;
       }
        if(fA ==2){
            new_filename=Form("./d2_pion_LO_noPt/EIC_A2_pion_10_100_skim%d_wide_noPt_new.root",j);
            t1->Add(new_filename.Data());
            cerr<<Form("-- Reading in file: %s", new_filename.Data())<<endl;
            new_filename=Form("./d2_pion_LO_noPt/EIC_A2_pion_10_100_skim%d_wide_noPt.root",j);
            t1->Add(new_filename.Data());
            cerr<<Form("-- Reading in file: %s", new_filename.Data())<<endl;
        }
        ...
        ofstream outf;
        if(fA ==12)
            outf.open(Form("c12_output_Q2_%d_lo_wide_noPT.dat",j));
        if(fA ==2)
            outf.open(Form("d2_output_Q2_%d_lo_wide_noPT.dat",j));
