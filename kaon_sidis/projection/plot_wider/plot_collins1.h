...
void plot(Int_t Ebeam=11, Int_t target_flag=3, Int_t i=1, Int_t j=1, Int_t k =1, Int_t flag_t=0, Int_t flag=0){
	...
	TString filename;
    if(particle_flag>0)
        filename.Form("../results_wider/%s_%s_E%d_%d_%d_hp.dat",target.Data(),particle.Data(),Ebeam,z_flag,Q2_flag);
    else 
        filename.Form("../results_wider/%s_%s_E%d_%d_%d_hm.dat",target.Data(),particle.Data(),Ebeam,z_flag,Q2_flag);
	...
	gStyle->SetOptStat(0);
	Int_t count1,count2;
	Double_t Q2[5000],x[5000],z[5000],pt[5000],y[5000],Astat[5000],N[5000],N_raw[5000],coverage[5000],coef[3][5000];
	Double_t temp;
	Int_t pt_flag = -1;
	Int_t ncount=0;
	...
	while(infile >> pt_flag >> count2){
		...
		for (Int_t j1=0;j1<count2;j1++){
			infile >> pt_temp >> x_temp >> z[ncount] >> Q2[ncount] >> pt[ncount] >> x[ncount] >> y[ncount] >> Astat[ncount] >> N[ncount]>> N_raw[ncount]>>
...
if(z_flag==8){
	...
	t1->DrawLatex(0.1,0.75,titlename);
}
else {
	titlename.Form("#scale[2.0]{%d < Q^{2} < %d}",Q2min,Q2max);
	t1->DrawLatex(0.35,0.90,titlename);
	titlename.Form("#scale[2.0]{%3.2f < z < %3.2f}",zmin,zmax);
	t1->DrawLatex(0.3,0.75,titlename);
}
