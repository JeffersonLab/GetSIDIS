const double max1 = 1.25;
const double min1 = -0.1;
double qmax1 = 0.25;
double qmin1 = -0.25;
...
void plot(Int_t i=1, Int_t j=1, Int_t k =1, Int_t flag_t=0, Int_t flag=0){
	...
	while(infile >> pt_flag >> count2){
		...
		for (Int_t j1=0;j1<count2;j1++){
			infile >> pt_temp >> x_temp >> z[ncount] >> Q2[ncount] >> pt[ncount] >> x[ncount] >> y[ncount] >> Astat[ncount] >> 
			...
			if(pt_temp==pt_flag && x_temp== j1){
				...
				//if (Astat[ncount]>0.&&Astat[ncount]<0.05&&coverage[ncount]>6.28/15.&&(coef[0][ncount]+coef[1][ncount]+coef[2][ncount])/3.<3.){ //&&Astat[ncount]<0.05
					...
					if (1){
					cerr<<Form("---z=%d, Q=%d pt = (%d) %f , x = (%d) %f, Asys= %f",z_flag,Q2_flag, pt_flag, pt[ncount],j1,x[ncount], Astat[ncount])<<endl;
