...
void plot(Int_t i=1, Int_t j=1, Int_t k =1, Int_t flag_t=0, Int_t flag=0){
  ...
  while(!infile.eof()){
    ...
    Astat[count] = Astat[count]*collins[count];
    if (Astat[count]<5e-2){
...
TH2F *h1 = new TH2F("h1","h1",100,0.03,0.56,100,min1,max1);
  h1->Draw();
  gStyle->SetOptStat(0);
  h1->SetTitle();
  ...
  h1->GetYaxis()->SetLabelColor(10);

  TGraphErrors *g1 = new TGraphErrors(count-1,x,pt,0,Astat);
  g1->Draw("*same");
  g1->SetMarkerColor(2);
  ...
  if (flag==1){
    TGaxis *axis = new TGaxis(0.03,min1,0.03,max1,qmin1,qmax1,506,"-R");
    if (i==1){
  ...
  tt->SetTextSize(0.03);
  tt->DrawLatex(0.05,0.1,"P_{T} #approx 0.1 GeV");
  tt->DrawLatex(0.05,0.3,"P_{T} #approx 0.3 GeV");
  tt->DrawLatex(0.05,0.5,"P_{T} #approx 0.5 GeV");
  tt->DrawLatex(0.05,0.7,"P_{T} #approx 0.7 GeV");
  tt->DrawLatex(0.05,0.9,"P_{T} #approx 0.9 GeV");
}
...
void plot_e06010(Int_t part=1, Int_t flag = 1, Int_t flag_6=0){
  ...
  // Float_t x[4]={0.136689,     0.18919,        0.249406,       0.336276};
   Double_t x[4]={(0.136926 + 0.136453) / 2+0.0204311, 
  ...
  Float_t Collins_pip_sys_err[4]={0.0317116,0.0432324,0.0287402,0.0525238};
  ...
  Float_t Collins_pim_sys_err[4]={0.024393,0.0337938,0.019797,0.0226173};
  

  Float_t Sivers_pip[4]={-0.0111463,-0.108013,-0.129014,-0.162731};
  Float_t Sivers_pip_err[4]={0.125489,0.138209,0.0964687,0.13306};
  Float_t Sivers_pip_sys_err[4]={0.0305844,0.0414134,0.0294927,0.0412472};
  ...
  Float_t Sivers_pim_sys_err[4]={0.0209133,0.0342015,0.0199649,0.0230208};
...
void plot_feng(Int_t part=1,Int_t flag_t = 0,Int_t flag_z = 0){
  ...
  if (part==1){
    partname = "pip_";
    FCollinsfile="FENG/Collins_pip.dat";
    FSiversfile="FENG/Sivers_pip.dat";
  }else if (part==2){
    partname = "pim_";
    FCollinsfile="FENG/Collins_pim.dat";
    FSiversfile="FENG/Sivers_pim.dat";
  }
...
void plot_pretz(Int_t part=1,Int_t flag_t = 0){
  ...
  while(!infile3.eof()){
    ...
    eff_y = 0.948319 -4.11327 * x_full[i_full] +11.7735* x_full[i_full]* x_full[i_full] -10.1719 * x_full[i_full]* x_full[i_full]* x_full[i_full];
    eff_factor = (1-eff_y)/(1-eff_y+eff_y*eff_y/2.);
    npip_full[i_full] *=eff_factor;
    npim_full[i_full] *=eff_factor;
  ...
  while(!infile4.eof()){
    ...
    eff_y = 0.948319 -4.11327 * x_pwave[i_pwave] +11.7735* x_pwave[i_pwave]* x_pwave[i_pwave] -10.1719 * x_pwave[i_pwave]* x_pwave[i_pwave]* x_pwave[i_pwave];
    eff_factor = (1-eff_y)/(1-eff_y+eff_y*eff_y/2.);
    npip_pwave[i_pwave] *=eff_factor;
    npim_pwave[i_pwave] *=eff_factor;
...
void plot_cal(Int_t target_flag, Int_t particle_flag, Int_t energy_flag,  Int_t z_flag,Int_t Q2_flag){
  Float_t factor,shift;
  factor=(max1-min1)/(qmax1-qmin1);
  shift=max1/factor-qmax1;

//   Double_t zero_point = shift;
//   Double_t scale = factor;
  TString filename,prefix;
  if (target_flag==1){
    prefix = "./calc2/proton/proton_";
//     prefix = "./calculation/proton/proton_";
  }else{
    prefix = "./calc2/neutron/neutron_"; 
//     prefix = "./calculation/neutron/neutron_"; 
  }
  
  filename.Form("%d_%d_%d.dat",energy_flag,Q2_flag,z_flag);
  filename = prefix + filename;
  cout << filename << endl;

  Double_t x[500],logx[500];
  Double_t unp[4][500],sivers[4][500],collins[4][500],pretz[4][500];

  ifstream infile(filename);
  for (Int_t i=0;i!=13;i++){
    infile >> x[i] >> unp[0][i] >> collins[0][i] >> sivers[0][i] >> pretz[0][i] >>unp[1][i] >> collins[1][i] >> sivers[1][i] >> pretz[1][i];
    //pi+
//     collins[0][i] = (collins[0][i]/unp[0][i])*scale + zero_point;
//     sivers[0][i] = (sivers[0][i]/unp[0][i])*scale + zero_point;
//     pretz[0][i] = (pretz[0][i]/unp[0][i])*scale + zero_point;

    collins[0][i] = (collins[0][i]/unp[0][i]+shift)*factor;
    pretz[0][i] = (pretz[0][i]/unp[0][i]+shift)*factor;
    
    //pi-
//     collins[1][i] = (collins[1][i]/unp[1][i])*scale + zero_point;
//     sivers[1][i] = (sivers[1][i]/unp[1][i])*scale + zero_point;
//     pretz[1][i] = (pretz[1][i]/unp[1][i])*scale + zero_point;
    collins[1][i] = (collins[1][i]/unp[1][i]+shift)*factor;
    pretz[1][i] = (pretz[1][i]/unp[1][i]+shift)*factor;
  ...
  if (particle_flag==1){
    //pi+
    // TGraph *g1 = new TGraph(13,x,collins[0]);
//     g1->Draw("Lsame");
//     g1->SetLineColor(1);
//     g1->SetLineWidth(3.0);


//     TGraph *g2 = new TGraph(100,logx,sivers[0]);
//     g2->Draw("Lsame");
//     g2->SetLineColor(2); 
//     g2->SetLineWidth(3.0);

    TGraph *g3 = new TGraph(13,x,pretz[0]);
    g3->Draw("Lsame");
    g3->SetLineColor(5);
    g3->SetLineWidth(3.0);
    ...
    // //pi-
    // TGraph *g1 = new TGraph(13,x,collins[1]);
//     g1->Draw("Lsame");
//     g1->SetLineColor(1);
//     g1->SetLineWidth(3.0);
     
//     TGraph *g2 = new TGraph(100,logx,sivers[1]);
//     g2->Draw("Lsame");
//     g2->SetLineColor(2);
//     g2->SetLineWidth(3.0);

    TGraph *g3 = new TGraph(13,x,pretz[1]);
    g3->Draw("Lsame");
    g3->SetLineColor(5);
    g3->SetLineWidth(3.0); 
  }
...
//   if (scale_flag==1){
//     TGaxis *axis = new TGaxis(0.0,llimit,0.0,hlimit,(llimit-zero_point)/scale,(hlimit-zero_point)/scale,504,"+L");
//     if (particle_flag==1){
//       axis->SetTitle("Asymmetry #pi^{+}");
//     }else{
//       axis->SetTitle("Asymmetry #pi^{-}");
...
//   if (z_flag==3&&Q2_flag==2){
//     TLegend *l1 = new TLegend(0.1,0.6,0.5,0.9);
//     l1->SetFillColor(0);
//     l1->SetTextSize(0.1);
// //     l1->AddEntry(g1,"Collins","l");
// //     l1->AddEntry(g2,"Sivers","l");
//     l1->AddEntry(g3,"Pretz","l");
//     l1->Draw("same");
//   }
