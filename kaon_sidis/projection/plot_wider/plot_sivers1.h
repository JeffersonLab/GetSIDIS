...
#define qmin1 -0.55;
int bin_count;
/*void plot1(Int_t z_flag =1, Int_t Q2_flag=1, Int_t flag_t=0, Int_t flag=0){{{*/
void plot1(Int_t i=1, Int_t j=1, Int_t k =1, Int_t flag_t=0, Int_t flag=0){
	...
	while(infile >> pt_flag >> count2){
		...
		for (Int_t j1=0;j1<count2;j1++){
            ...
            event_count+=pow(1./Astat[ncount],2);
...
if(z_flag==8){
	...
	t1->DrawLatex(0.15,0.9,titlename);
	titlename.Form("#scale[2.0]{%3.2f < z < %3.2f}",zmin,zmax);
	t1->DrawLatex(0.1,0.75,titlename);
}
else {
	titlename.Form("#scale[2.0]{%d < Q^{2} < %d}",Q2min,Q2max);
	t1->DrawLatex(0.36,0.90,titlename);
	titlename.Form("#scale[2.0]{%3.2f < z < %3.2f}",zmin,zmax);
	t1->DrawLatex(0.31,0.75,titlename);
}
