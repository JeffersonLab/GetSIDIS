...
int main(){
	...
	Int_t particle_flag = 1; //cerr<<"--- particle (1->pip, 2->pim) = "; cin >> particle_flag;
	Int_t z_flag = 0;
	Int_t Q2_flag = 0;
	int err = -1000;
    bininfo.open(Form("bin_info_%d_%d.txt",target_flag, particle_flag));
    bininfo <<Form("%8s %8s %8s %8s %8s %8s %8s %8s","Q2l", "Q2u","zl", "zu","Ptl", "Ptu","xl", "xu")<<endl;
	for(int i=1;i<=8;i++){
			for(int j=1;j<=6;j++){
				z_flag = i;
				Q2_flag = j;
...
//	const double dilute_factor = 0.85; //85% overall efficiency,1/3 for He3 
	...
	/*Count How many pt bins{{{*/
	TH1F *h0  =new TH1F("h0","h0",1000,PT_MIN,PT_MAX);
	TString cut;
	double pt_cut[1000];
	double single_bin_pt = 5.e6/det_eff;
	int count = 0;
    ...
    count++;
    for(Int_t i=0;i<8;i++){
      cut = Form("weight*(pt>%f&&pt<%f)",0.2*i,0.2*(i+1));
	  T->Project("h0","pt",cut);	  
	  pt_cut[count]=0.2*(i+1);
	  //cerr<<Form("---#%d pt[%d]=%f N=%f",i,count,pt_cut[count],h0->GetSum())<<endl;
	  if(h0->GetSum()>single_bin_pt){
		count++;
	  }
	  h0->Reset();
	}	
	...
	//	if (xbin<1) xbin = 1; //Counts as one bin if the statistic is really low
		...
		for (Int_t j=0;j<xbin;j++){
            ...
            bininfo<<Form("%8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f"
                    , Q2min, Q2max, zmin, zmax, ptmin, ptmax, xmin, xmax)<<endl;

            cout<<Form("%8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f"
                    , Q2min, Q2max, zmin, zmax, ptmin, ptmax, xmin, xmax)<<endl;
