...
int makebin(int target_flag, int particle_flag){
	...
	outf_total << "\t" << xbin << endl;
	cerr    << Form("--- Total x-bin is %d with N=%e .vs. %e" ,xbin,single_bin_raw, h2->GetSum()/100) << endl;
	TString histoname;
	xmin = X_MIN;
	/*Binning in X{{{*/
	for (Int_t i=0;i<xbin;i++){
		...
		cerr<<Form("---- Working on x=(%d)%f, N=%e",i,h1x->GetMean(),N_out)<<endl;
		if(N_out>0.){
			outf_total << 0 <<" \t" << i << " \t"
				<< h1z->GetMean() << " \t" 
				<< h1Q2->GetMean() << " \t" 
