...
#include "TCanvas.h"
#include "Rtypes.h"
#include "sole_inter.h"
#include "iostream"
#include "math.h"
using namespace std;
//initialize the number of bins and evnets of the histogram
void sole_inter::init(Int_t nbins, Double_t* events){ //all phis(degree)
  Nbins = nbins;
  Bsize = 360./Nbins;
  if (Nbins>1000){
...
Int_t sole_inter::findmin(Int_t current_bin,Double_t current_angle){
...
Int_t sole_inter::findmax(Int_t current_bin, Double_t current_angle){
...
void sole_inter::adjust(Int_t current_bin,Double_t current_angle,Int_t minbin,Int_t maxbin){
...
void sole_inter::get_new(Double_t *y){
  for(Int_t i=0;i!=Nbins;i++){
    *(y+i) = x[i];
  }
}
...
void sole_inter::get_limit(Double_t *y){
  for (Int_t i=0;i!=4;i++){
    *(y+i) = anglelimit[i];
  }
}

Double_t sole_inter::get_accep(Double_t x){ //x is angle(degree)
