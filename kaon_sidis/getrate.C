...
int main(Int_t argc, char *argv[]){
    ...
    Double_t rate_hp[7] ={7*0.0}, rate_hm[7]={7*0.0};
    ...
    for (Int_t i=0;i!=T0->GetEntries();i++){/*{{{*/
        ...
        if(Q2>1.0&&W>2.3&&Wp>1.6&&z>0.3&&z<0.7&&mom_had>7.&&mom_had<=10.){
            rate_hp[0] += weight_hp * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hp);
            rate_hm[0] += weight_hm * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hm);
        }
        if(Q2>1.0&&W>2.3&&Wp>1.6&&z>0.3&&z<0.7&&mom_had>6.0&&mom_had<=7.0){
            rate_hp[1] += weight_hp * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hp);
            rate_hm[1] += weight_hm * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hm);
        }
        if(Q2>1.0&&W>2.3&&Wp>1.6&&z>0.3&&z<0.7&&mom_had>5.0&&mom_had<=6.0){
            rate_hp[2] += weight_hp * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hp);
            rate_hm[2] += weight_hm * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hm);
        }
        if(Q2>1.0&&W>2.3&&Wp>1.6&&z>0.3&&z<0.7&&mom_had>4.0&&mom_had<=5.0){
            rate_hp[3] += weight_hp * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hp);
            rate_hm[3] += weight_hm * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hm);
        }
        if(Q2>1.0&&W>2.3&&Wp>1.6&&z>0.3&&z<0.7&&mom_had>3.0&&mom_had<=4.0){
            rate_hp[4] += weight_hp * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hp);
            rate_hm[4] += weight_hm * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hm);
        }
        if(Q2>1.0&&W>2.3&&Wp>1.6&&z>0.3&&z<0.7&&mom_had>2.0&&mom_had<=3.0){
            rate_hp[5] += weight_hp * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hp);
            rate_hm[5] += weight_hm * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hm);
        }
        if(Q2>1.0&&W>2.3&&Wp>1.6&&z>0.3&&z<0.7&&mom_had>1.0&&mom_had<=2.0){
            rate_hp[6] += weight_hp * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hp);
            rate_hm[6] += weight_hm * luminosity * (acc_f_ele+acc_l_ele)*(acc_f_hm);
        }
    }
    delete T0;/*}}}*/

    ofstream outf(Form("mom_rate_%s_%s_%d.txt", target.Data(), particle.Data(), Ebeam));
    ...
    for(int i=0;i<7;i++){
        cout<<Form("--- Hadron+ (%d<Ph<%d GeV/c): Rate = %f", i+1, i+2,rate_hp[6-i])<<endl;
        outf<<Form("--- Hadron+ (%d<Ph<%d GeV/c): Rate = %f", i+1, i+2,rate_hp[6-i])<<endl;
    }
    cout<<endl;
    outf<<endl;

    for(int i=0;i<7;i++){
        cout<<Form("--- Hadron- (%d<Ph<%d GeV/c): Rate = %f", i+1, i+2,rate_hm[6-i])<<endl;
        outf<<Form("--- Hadron- (%d<Ph<%d GeV/c): Rate = %f", i+1, i+2,rate_hm[6-i])<<endl;
    }
    outf.close();
